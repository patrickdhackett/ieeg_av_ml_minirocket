...
## FILTER REGIONS TO THOSE THAT OVERLAP IN 5 PEOPLE OR MORE
...
for subject in top_group:
    ...
    valid_trials = [
        ...
        if not (np.isnan(trialinfo[t,0]) or np.isnan(trialinfo[t,6])) and int(trialinfo[t,0]) + 5500 <= data.shape[2]
    ]
    ...
    for ch in roi_chs:
        ...
        for t in valid_trials:
            enc0 = int(trialinfo[t,0])
            ty = int(trialinfo[t,6])
            tslice = data[t, ch, enc0:enc0+5500]
...
print(results_df)
print(results_df.sort_values(by='ValidationAccuracy', ascending=False))
...
#  save
results_df.to_csv(os.path.join(data_dir, 'smartminirocket_channel_val_accuracies_model2.csv'),
                  index=False)
...
for idx, row in filtered_df.iterrows():
    ...
    try:
        ...
        valid_trials = [
            ...
            and int(trialinfo[t, 0]) + 5500 <= data.shape[2]
        ]
        train_trials, temp_trials = train_test_split(
        ...
        cm = confusion_matrix(y_test, y_test_pred, labels=sorted(np.unique(y_test)))
        tp = np.diag(cm)
        fn = cm.sum(axis=1) - tp
        fp = cm.sum(axis=0) - tp
        tn = cm.sum() - (tp + fn + fp)
...
print(sig_df)
sig_df.to_csv(os.path.join(data_dir, 'smartminirocket_channel_selective_channels_model2.csv'),
                  index=False)

filtered_df.to_csv(os.path.join(data_dir, 'smartminirocket_channel_val_accuracies_model2_test.csv'),
                  index=False)