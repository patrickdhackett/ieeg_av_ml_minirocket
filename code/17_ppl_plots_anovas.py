...
OUTPUT_DIR        = r"E:\data\project_repos\phzhr_turtles_av_ml\model_results\17_plots_ppl_gamma"
...
if __name__ == "__main__":
    ...
    #
    #    subj_gamma_perclass[(r1,r2)][cls] = set of subjects that contributed gamma‐only for that class
    #    combo_gamma_perclass[(r1,r2)][cls] = set of combo indices gamma‐only for that class
    #
    data_all              = {}
    data_gamma            = {}
    subj_all              = {}
    ...
    for fname in mat_files:
        ...
        # 5a) Drop any trial that has a NaN anywhere in trialinfo
        ...
        for (r1, r2) in roi_pairs:
            ...
            # If a band has no matching freq bins, we'll skip it below.
            ...
            for fb, fidx in band_indices.items():
                ...
                for tw in TIME_WINDOWS:
                    ...
                    for cls in [1, 2, 3, 4]:
                        ...
                        with warnings.catch_warnings():
                            warnings.simplefilter("ignore", category=RuntimeWarning)
                            mean_per_gamma = np.nanmean(submat_gamma, axis=0)  # (nGammaCombos,)
    ...
    # ───────────────────────────────────────────────────────────────────────────────
    #  6) Plotting & ANOVA (side-by-side: All Channels vs. Gamma-Only)
    # ───────────────────────────────────────────────────────────────────────────────
    total_plots = 0
    for (r1, r2), _ in data_all.items():
        for fb in FREQ_BANDS.keys():
            ...
            # Check if there is any data at all in either “all” or “gamma”
            ...
            if not (has_any_all or has_any_gamma):
            ...
            for i, cls in enumerate(class_labels):
                ...
                for j in range(n_windows):
                    ...
                    ax.scatter(
                        ...
                        vals,
                        s=6,                   # a bit darker than before
                        marker=".",            # small dot
                        color="#888888",       # slightly darker gray
                        alpha=0.35             # more opaque
                    )

            # Show “n_subj” and “n_combo” at the top‐left of the axes (axes‐coordinates)
            ...
            # Run one‐way ANOVA across classes at each time window (all channels)
            for j, tw in enumerate(time_order):
            ...
            textstr_g = "\n".join(info_lines)
            ax.text(0.02, 0.98, textstr_g, transform=ax.transAxes,
                    fontsize=8, va='top', ha='left')

            # ANOVA stars (gamma‐only) at each time window
            for j, tw in enumerate(time_order):
            ...
            ax.set_xticklabels(["Encoding", "Delay1", "Delay2", "Delay3"])
            ax.set_xlabel("Time Window")
            ax.set_title("Gamma-Only Channels")
            ax.legend(title="Class", loc="upper right", fontsize="small")
            ax.axhline(0, color="gray", linewidth=1)
