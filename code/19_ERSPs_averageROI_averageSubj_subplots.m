...
for i = 34:35
    ...
    cb = colorbar('Position',[0.95 0.11 0.015 0.77]);
    ylabel(cb,'Z-scored Power (baseline 1–1.5 s)','FontSize',11);
    
    uniqueChanCount = numel(unique(allChans));      % how many distinct channels survived
    uniqueSubjCount = numel(unique(allSubjects));   % how many distinct subjects had at least one channel
    
    sgtitle({[ROI{1} '  –  Group ERSP (2 × 2 classes)'], ...
             [sprintf('%d chans across %d subjects', uniqueChanCount, uniqueSubjCount)]}, ...
