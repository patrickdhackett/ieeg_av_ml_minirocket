...
for i = 1:height(subjects)
    ...
    for L = 1:4
        ...
        if ~isempty(freqPerChan)
            ...
            % Compute average across channels (first dimension)
            avgPow = mean(powArray, 1, 'omitnan');  % [1 x F x T]
            
            % Create averaged freq struct
            freqAvg = freqPerChan{1};  % copy metadata
            freqAvg.powspctrm = avgPow;
            freqAvg.label = {['ROI average: ' ROI{1}]};  % label for the average
            ...
            ft_singleplotTFR(cfg, freqAvg);
                        % Overlay dashed lines and labels
            hold on;
            h = colorbar;
            ylabel(h, 'Z-scored Power (relative to first 500 ms of Fix)', 'FontSize', 11);
            title({[ROI{1} ' ' subject ' ' session ' ' ClassLookup{L}], ...
                [num2str(length(chanIdx)) ' chans']});
            yl = ylim;  % get current y-limits of the plot
            
            % Encoding marker at 2s
            xline(2, '--k', 'Encoding', 'LabelOrientation', 'horizontal', ...
                  'LabelVerticalAlignment', 'bottom', 'LabelHorizontalAlignment', 'right', 'FontSize', 10);
            
            % Delay Period marker at 3.5s
