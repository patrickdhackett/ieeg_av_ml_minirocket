...
function [B,T,F]=BOSC_tf_power(eegsignal,F,Fsample,wavenumber);
% [B,T,F]=BOSC_tf_power(eegsignal,F,Fsample,wavenumber);
%
...
st=1./(2*pi*(F/wavenumber)); % standard deviation % determines the temporal width of the Gaussian envelope.
A=1./sqrt(st*sqrt(pi)); % Normalization Constant % ensures that the energy of the wavelet is normalized
B = zeros(length(F),length(eegsignal)); % initialize the time-frequency matrix
for f=1:length(F) % loop through sampled frequencies
  t=-3.6*st(f):(1/Fsample):3.6*st(f); % This time interval covers most of the Gaussian’s energy (since a Gaussian decays rapidly away from its mean)
  m=A(f)*exp(-t.^2/(2*st(f)^2)).*exp(i*2*pi*F(f).*t); % Morlet wavelet % the real and imaginary parts represent a cosine and sine modulated by a Gaussian window
  y=conv(eegsignal,m);  % Convolution serves as a sliding “match filter” that checks, at every time point, how well the wavelet (of a given frequency) “matches” the EEG data
  y=abs(y).^2; % yields the power (or energy) at that particular frequency and at each time point
