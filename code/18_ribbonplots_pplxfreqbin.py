...
OUTPUT_DIR          = r"E:\data\project_repos\phzhr_turtles_av_ml\model_results\18_ppl_ribbon_pplxfreqbin\peak40hz"
...
if __name__ == "__main__":
    ...
    for (r1, r2) in region_pairs:
        ...
        for cls in classes:
            ...
            # 1) Find the first index where frequency ≤40 Hz:
            idx_start = np.where(freqs <= 40)[0][0]   # e.g. idx_start == 18
            
            # 2) In your plotting loop, do:
            ax.plot(
                freqs[idx_start:], 
                mean_vec[idx_start:], 
            ...
            ax.fill_between(
                freqs[idx_start:],
                mean_vec[idx_start:] - sem_vec[idx_start:],
                mean_vec[idx_start:] + sem_vec[idx_start:],
                alpha=0.3
            )
