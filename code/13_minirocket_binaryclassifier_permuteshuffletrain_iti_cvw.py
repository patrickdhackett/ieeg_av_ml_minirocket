...
n_cores           = 48          # number of parallel workers
n_perm            = 1000        # permutations per channel/class
...
def process_channel(subject, data, tri, region_labels, valid_trials, ch):
    ...
    Windows (all length=window_len):
      ...
      • Delay1:   trialinfo[:,4] +100ms → +1500ms  
      • Delay2:   trialinfo[:,4] +1600ms → +1500ms  
    """
    out = []
    try:
        # extract the four windows
        ...
        X_d2,  y_d2  = extract_window(data, tri, valid_trials, ch,
                                      sc=4, length=window_len, offset=100+window_len)
        
        
        if (len(y_enc) < 5 or len(set(y_enc)) < 2
            or len(y_fix) < 5
            or len(y_d1)  < 5):
            return []   # exactly how old code would drop it
        
        
        # skip if too few trials anywhere
        if (len(y_enc) < 5 or len(set(y_enc)) < 2
            or len(y_fix) < 5 or len(y_d1) < 5 or len(y_d2) < 5):
            return out

        # loop each one-vs-all class
        for L in np.unique(y_enc):
            ...
            # instantiate & fit a fresh MiniRocket for this split
            rk = MiniRocket(num_kernels=10000, random_state=42)
            rk.fit(Xb)

            # do all 5 transforms once
            ...
            # build permutation nulls (only retrain Ridge)
            ...
            # paired delay vs fix tests
            obs_d1   = a_d1 - a_fx
            obs_d2   = a_d2 - a_fx
            null_pd1 = null_d1 - null_fx
            null_pd2 = null_d2 - null_fx
            p_d1fx = (null_pd1 >= obs_d1).sum()+1; p_d1fx /= (n_perm+1)
            p_d2fx = (null_pd2 >= obs_d2).sum()+1; p_d2fx /= (n_perm+1)
        ...
        print(f"Warning: subject={subject} ch={ch} error: {e}")
...
pd.DataFrame(results).to_csv(
    os.path.join(outdir, '13_onevsall_with_permutationontrain_iti.csv'),
    index=False
)
