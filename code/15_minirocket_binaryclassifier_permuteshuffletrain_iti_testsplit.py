...
n_cores           = 16             # parallel workers
n_perm            = 100             # *** only 10 permutations now ***
alpha             = 0.05           # significance threshold
window_len        = 1500           # ms per window
test_size         = 0.2            # fraction of trials held out for testing
...
def process_channel(subject, data, tri, region_labels, valid_trials, ch):
    """
    One‐vs‐all miniROCKET + Ridge, with a held‐out test split per class
    and permutation‐shuffled training‐label nulls.

    Parameters
    ----------
    subject        : str
    data           : array, shape (n_trials, n_chans, n_times)
    tri            : array, shape (n_trials, >=8) trialinfo
    region_labels  : list of channel→ROI strings
    valid_trials   : list of trial indices to keep
    ch             : int, channel index

    Returns
    -------
    out : list of dicts (one per class L that meets criteria)
    """
    out = []
    try:
        # 1) extract windows for *all* valid_trials
        ...
        # 2) need at least 5 enc trials, ≥2 classes, 5 fix & 5 d1 trials
        if (len(y_enc) < 5
            or len(set(y_enc)) < 2
            or len(y_fix) < 5
            or len(y_d1)  < 5):
        ...
        for L in np.unique(y_enc):
            ...
            # 3a) downsample negatives to balance
            try:
            ...
            yb   = (y_enc[keep] == L).astype(int)

            # 4) split KEEP into train / test
            ...
            y_train = (y_enc[train_idx] == L).astype(int)

            # build train & test sets for each window
            X_train_enc  = X_enc[train_idx]
            X_fix_test   = X_fix[test_idx]
            X_enc_test   = X_enc[test_idx]
            X_d1_test    = X_d1[test_idx]
            X_d2_test    = X_d2[test_idx]
            y_fix_test   = (y_fix[test_idx] == L).astype(int)
            y_enc_test   = (y_enc[test_idx] == L).astype(int)
            y_d1_test    = (y_d1[test_idx]  == L).astype(int)
            y_d2_test    = (y_d2[test_idx]  == L).astype(int)

            # 5) fit miniROCKET on the training encoding epochs
            rk = MiniRocket(num_kernels=10000, random_state=42)
            rk.fit(X_train_enc)
            feat_train    = rk.transform(X_train_enc)
            feat_fix_test = rk.transform(X_fix_test)
            feat_enc_test = rk.transform(X_enc_test)
            feat_d1_test  = rk.transform(X_d1_test)
            feat_d2_test  = rk.transform(X_d2_test)

            # 6) train & eval Ridge on the held‐out test set
            clf = RidgeClassifierCV(alphas=np.logspace(-3,3,10))
            clf.fit(feat_train, y_train)
            a_fx = accuracy_score(y_fix_test, clf.predict(feat_fix_test))
            a_en = accuracy_score(y_enc_test, clf.predict(feat_enc_test))
            a_d1 = accuracy_score(y_d1_test,  clf.predict(feat_d1_test))
            a_d2 = accuracy_score(y_d2_test,  clf.predict(feat_d2_test))

            # 7) build permutation nulls (shuffle only the train labels)
            null_fx = np.zeros(n_perm)
            null_en = np.zeros(n_perm)
            null_d1 = np.zeros(n_perm)
            null_d2 = np.zeros(n_perm)
            for i in range(n_perm):
                y_train_sh = np.random.permutation(y_train)
                perm = clone(clf)
                perm.fit(feat_train, y_train_sh)
                null_fx[i] = accuracy_score(y_fix_test, perm.predict(feat_fix_test))
                null_en[i] = accuracy_score(y_enc_test, perm.predict(feat_enc_test))
                null_d1[i] = accuracy_score(y_d1_test,  perm.predict(feat_d1_test))
                null_d2[i] = accuracy_score(y_d2_test,  perm.predict(feat_d2_test))
            ...
            # 9) permutation p‐values (one‐sided)
            ...
            p_d1fx   = (null_pd1 >= obs_d1).sum()+1
            p_d2fx   = (null_pd2 >= obs_d2).sum()+1
            p_d1fx  /= (n_perm+1)
            p_d2fx  /= (n_perm+1)
            ...
            p_fx_bin = btest(clf.predict(feat_fix_test), y_fix_test)
            p_en_bin = btest(clf.predict(feat_enc_test), y_enc_test)
            p_d1_bin = btest(clf.predict(feat_d1_test),  y_d1_test)
            p_d2_bin = btest(clf.predict(feat_d2_test),  y_d2_test)
...
# ── SAVE RESULTS ─────────────────────────────────────────────────────────────
outdir = os.path.join(MODEL_RESULTS_DIR, '15_permutationshuffledtrain_iti_testsplit')
os.makedirs(outdir, exist_ok=True)
pd.DataFrame(results).to_csv(
    os.path.join(outdir, '15_onevsall_with_permutationontrain_iti_testsplit_20percent.csv'),
    index=False
)
