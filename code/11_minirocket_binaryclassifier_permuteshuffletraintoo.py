...
# ── USER PARAMETERS ───────────────────────────────────────────────────────────
DATA_DIR          = r'E:\data\project_repos\phzhr_turtles_av_ml\data\without_iti'
...
def process_channel(subject, data, tri, region_labels, valid_trials, ch, window_len):
    ...
    try:
        ...
        for L in np.unique(y_tr):
            ...
            # — train the real pipeline —
            real_pipe = make_pipeline(
            ...
            y_pred_fix = real_pipe.predict(X_val_fix)
            y_pred_enc = real_pipe.predict(X_val_enc)
            y_pred_dp  = real_pipe.predict(X_val_dp)
            ...
            # — build permutation nulls by retraining on shuffled train-labels —
            null_fx = np.zeros(n_perm)
            null_en = np.zeros(n_perm)
            null_dp = np.zeros(n_perm)
            for i in range(n_perm):
                yb_sh = np.random.permutation(yb)
                perm_pipe = clone(real_pipe)  # clone both MiniRocket & classifier
                perm_pipe.fit(Xb, yb_sh)
                null_fx[i] = accuracy_score((y_val_fix == L).astype(int),
                                            perm_pipe.predict(X_val_fix))
                null_en[i] = accuracy_score((y_val_enc == L).astype(int),
                                            perm_pipe.predict(X_val_enc))
                null_dp[i] = accuracy_score((y_val_dp  == L).astype(int),
                                            perm_pipe.predict(X_val_dp))
            ...
            # — permutation p-values (one-sided) —
            p_fx_perm = (null_fx >= acc_fx).sum()+1; p_fx_perm /= (n_perm+1)
            p_en_perm = (null_en >= acc_en).sum()+1; p_en_perm /= (n_perm+1)
            p_dp_perm = (null_dp >= acc_dp).sum()+1; p_dp_perm /= (n_perm+1)

            # — paired Delay vs Fix test —
            ...
            p_fx_bin = binomtest(k_fx, len(y_val_fix), p=0.5,
                                 alternative='greater').pvalue
            k_en = (y_pred_enc == (y_val_enc == L).astype(int)).sum()
            p_en_bin = binomtest(k_en, len(y_val_enc), p=0.5,
                                 alternative='greater').pvalue
            k_dp = (y_pred_dp == (y_val_dp  == L).astype(int)).sum()
            p_dp_bin = binomtest(k_dp, len(y_val_dp), p=0.5,
                                 alternative='greater').pvalue
...
for idx, subject in enumerate(top_group, start=1):
    ...
    n_chans = len(roi_chs)
    print(f"    → {n_chans} channels to process", flush=True)

    # wrap the Parallel call in a fresh tqdm_joblib context:
    with tqdm_joblib(
        tqdm(desc=f"{subject} channels", total=n_chans, ncols=80)
    ):
        channel_results = Parallel(
            n_jobs=n_cores,
            backend='loky'
        )(
...
# ── SAVE ─────────────────────────────────────────────────────────────────────
os.makedirs(os.path.join(MODEL_RESULTS_DIR, '11_permutationshuffledtrain_test'), exist_ok=True)
pd.DataFrame(results).to_csv(
    os.path.join(MODEL_RESULTS_DIR, '11_permutationshuffledtrain/11_onevsall_with_permutationontrain.csv'),
    index=False
)
